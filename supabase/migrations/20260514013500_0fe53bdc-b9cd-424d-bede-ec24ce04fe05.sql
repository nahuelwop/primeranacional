...
DROP POLICY IF EXISTS "own progress select" ON public.tournament_progress;
CREATE POLICY "own progress select"
ON public.tournament_progress
FOR SELECT
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "own progress insert" ON public.tournament_progress;
CREATE POLICY "own progress insert"
ON public.tournament_progress
FOR INSERT
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "own progress update" ON public.tournament_progress;
...
DROP POLICY IF EXISTS "admins logos insert" ON storage.objects;
...
DROP POLICY IF EXISTS "admins logos update" ON storage.objects;
...
DROP POLICY IF EXISTS "admins logos delete" ON storage.objects;
