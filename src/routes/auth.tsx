...
function AuthPage() {
  ...
  async function handle(e: React.FormEvent) {
    ...
    try {
      ...
      if (!cleanUsername.match(/^[a-zA-Z0-9_]{3,24}$/)) {
        throw new Error("Usuario: 3-24 letras, números o _");
      }
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 max-w-md mx-auto w-full px-4 py-10">
          ...
          <div className="mt-6 rounded-xl bg-card border border-border p-5 space-y-2">
            ...
            {isAdmin && (
              <div className="text-celeste font-display">
                ⭐ ADMIN
              </div>
            )}
            ...
            {isAdmin && (
              <Link
                to="/admin"
                className="block mt-2 text-celeste underline"
              >
                Panel admin →
              </Link>
            )}
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-md mx-auto w-full px-4 py-10">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Tu progreso del torneo se guarda en tu cuenta.
        </p>
