...
function CarreraPage() {
  ...
  async function onMatchEnd(lg: number, rg: number, _stats: MatchStats) {
    ...
    for (const key of checkCareerAchievementKeys(next, nextBudget)) {
      await tryUnlock(key);
    }
    ...
    if (isSeasonFinished(next)) {
      ...
      if (champ === teamId) {
        await tryUnlock(next.zone === "A" ? "campeon_zona_a" : "campeon_zona_b");
      }
    }
  ...
  async function onSimulateMatch() {
    ...
    const { hg, ag } = applyTeamRatingsTemporarily(
      state.teamRatings,
      () => simulateMatch(nextMatch.home, nextMatch.away)
    );

    await onMatchEnd(
      nextMatch.home === teamId ? hg : ag,
      nextMatch.home === teamId ? ag : hg,
      {} as MatchStats
    );
  }
  ...
  async function onActivateCorruption(kind: typeof CORRUPTION_CATALOG[number]["kind"]) {
    ...
    if (!confirm(
      `¿Activar "${opt.name}"?\n${opt.desc}\nCosto: $${opt.cost} · Penalidad ingresos: -${opt.penaltyPct}% por ${opt.matches} fechas.`
    )) return;
  ...
  function seedRegionalPlayoffFromState(source: CareerState, userId: string) {
    ...
    const roster =
      source.leagueRosters?.regional_federal_amateur ??
      getTeamsByDivision("regional_federal_amateur").map(t => t.id);
    ...
    const unique = Array.from(
      new Map(seeds.map(r => [r.teamId, r])).values()
    ).sort(
      (a, b) =>
        (b.pts / Math.max(1, b.pj)) -
          (a.pts / Math.max(1, a.pj)) ||
        b.dg - a.dg ||
        b.pts - a.pts
    );
    ...
    const opponentCandidates = (
      ...
      .filter(
        (m): m is NonNullable<typeof m> =>
          !!m && m.region !== myMeta.region
      )
      .map(m => allRows.find(r => r.teamId === m.id))
      .filter(Boolean) as StandingRow[];

    let nationalOpponent =
      sortStandings(opponentCandidates)[0]?.teamId;
  ...
  async function advanceSeason() {
    ...
    try {
      ...
      const needPlayoff = getCareerPlayoffNeed(
        completed,
        teamId
      );
      ...
      if (needPlayoff) {
        ...
        if (tournamentMatches) {
          ...
          if (
            ...
            const regionalChampion =
              tournamentState.regionalChampion;

            const nationalFinal =
              tournamentState.regionalNationalFinal;

            const regionalWon =
              regionalChampion === teamId;

            const nationalFinished =
              !!nationalFinal?.winner;

            if (!regionalWon) {
              // Si otro club ganó la etapa regional,
              // la temporada ya puede cerrarse.
        ...
        fresh.totalGoalsScored =
          completed.totalGoalsScored;

        fresh.bestUnbeaten =
          completed.bestUnbeaten;

        fresh.streakUnbeaten = 0;

        fresh.zoneChampions = [
          ...completed.zoneChampions,
        ];

        fresh.seasonHistory = [
          ...(completed.seasonHistory ?? []),
        ];
        ...
        fresh.sponsor =
          completed.sponsor ?? null;

        fresh.stadiumUpgrades =
          completed.stadiumUpgrades;
        ...
        fresh.totalWins =
          completed.totalWins ?? 0;

        fresh.totalDraws =
          completed.totalDraws ?? 0;

        fresh.totalLosses =
          completed.totalLosses ?? 0;
        ...
        for (
          const key of checkCareerAchievementKeys(
            completed,
            budget
          )
        ) {
      ...
      // Temporada normal: resolver ascensos/descensos
  ...
  if (
      ...
      return (
        <Shell>
          <div className="p-16 text-center text-destructive">
            No se pudo cargar uno de los equipos
            de esta división.
          </div>
        </Shell>
      );
    ...
    const fx =
      currentCorruptionEffects(state);

    return (
      <Shell musicActive={false}>
        <div className="text-center text-sm text-muted-foreground mb-2">
          Temporada {season} · Fecha{" "}
          {nextMatch.round} ·{" "}
          {userIsHome
            ? "Local"
            : "Visitante"}
        ...
        <Game
          ...
          cancelOpponentGoals={
            fx.cancelOpponentGoals ?? 0
          }
          doubleGoalChance={
            fx.doubleGoalChance ?? 0
          }
          onEnd={onMatchEnd}
          onExit={() =>
            setPlaying(false)
      ...
      <Shell>
        <div className="hud-panel p-6">
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Elegí una categoría y el club que
            vas a dirigir. Los ascensos,
            descensos y temporadas quedan
            guardados.
          </p>
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  Carrera completa con tabla anual
                  y promedios.
                </div>
              </div>

              <span className="text-xs text-muted-foreground">
                {firstDivisionTeams.length} clubes
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-2">
              {firstDivisionTeams.map(t => (
                ...
                >
                  ...
                  <div className="text-xs mt-2 font-display truncate">
                    {t.short}
                  </div>
          ...
          <div className="mt-8 pt-6 border-t border-border/60">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  Modo carrera original por zonas.
                </div>
              </div>
          ...
          <div className="mt-8 pt-6 border-t border-border/60">
            <div className="font-display text-2xl text-celeste mb-1">
              OTRAS CATEGORÍAS
            </div>

            <div className="text-xs text-muted-foreground mb-4">
              Empezá desde Primera B, Primera C,
              Federal A o Regional Federal Amateur.
            </div>
  ...
  return (
    <Shell hideNav>
      ...
      <header className="hud-in grid grid-cols-[auto_1fr_auto] items-center gap-4 border-b border-border/50 pb-0 mb-4">
        <Link
          to="/"
          className="flex items-center gap-2.5 pb-3 min-w-0"
        >
          ...
          <span className="hidden sm:block leading-none">
            <span className="block font-display text-sm tracking-widest">
              PRIMERA
            </span>

            <span className="block font-display text-sm tracking-widest text-muted-foreground">
              {COMPETITIONS[
                division
        ...
        <div className="flex items-center gap-3 pb-3 shrink-0">
          <div className="text-right hidden sm:block leading-tight">
            <div className="font-display text-sm tracking-wide truncate max-w-[170px]">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2.5">
        ...
        <span className="hud-card px-3 py-1.5 flex items-center gap-1.5 text-xs">
          ...
          <span className="font-display tracking-wide tabular-nums">
            $
            <CountUp
              value={budget}
            />
      ...
      >
        {tab ===
          ...
          <InicioTab
            ...
            /* CORRECCIÓN:
               ahora InicioTab recibe el estado de transición */
            transitioning={
              transitioning
            }
...
function InicioTab({
  ...
  /* CORRECCIÓN:
     la variable ahora existe dentro de InicioTab */
  transitioning,
  ...
  const side = [
    ...
    {
      k: "universo" as TopTab,
      icon: "🌎",
      title:
        "Universo",
      sub:
        "Mercado, ranking y desafíos",
    },
  ];
  ...
  const objetivos =
    ...
    "primera_division"
      ? [
          {
            label:
              "Terminar entre los 10 primeros",
            done:
              pos > 0 &&
              pos <= 10,
          },
          ...
          {
            label:
              "Mantener la valla invicta en 6 partidos",
            done:
              state.matches.filter(
                m =>
                  m.played &&
    ...
    <div className="grid lg:grid-cols-[250px_minmax(0,1fr)_340px] gap-3 items-start">
        ...
        {side.map(
          (i, idx) => (
            <button
              ...
              className={`hud-rise w-full text-left px-4 py-3 flex items-center gap-3 hud-card ${
                idx === 0
                  ? "hud-card-active"
                  : ""
              }`}
      ...
      <section className="order-1 lg:order-2 space-y-3">
        <div className="hud-in match-hero relative overflow-hidden rounded-[0.9rem] border border-border/50">
          ...
          <div
            className="match-hero-smoke"
            aria-hidden="true"
          />

          <div
            className="match-hero-light"
            aria-hidden="true"
          />

          <div
            className="match-hero-grass"
            aria-hidden="true"
          />
          ...
          <div className="relative p-5">
            {nextMatch ? (
              ...
              <div className="text-center py-6">
                ...
                {(() => {
                  const relegated =
                    divisionRelegationCandidates(
                      state
                    );

                  const promoted =
                    divisionPromotionCandidates(
                      state
                    );
                ...
                <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
                  ...
                  {/* CORRECCIÓN:
                      ahora sí encuentra transitioning */}
                  <button
                    onClick={
                      onAdvance
                    }
                ...
                {division ===
                  ...
                  firstDivisionRelegated(
                    ...
                    <div className="text-[11px] text-destructive mt-3 max-w-sm mx-auto">
                      ...
                      esta división se combinan
                      la tabla anual y los
                      promedios.
                    </div>
                  )}
                ...
                {division !==
                  ...
                  divisionRelegationCandidates(
                    ...
                    <div className="text-[11px] text-destructive mt-3 max-w-sm mx-auto">
                      Tu club ocupa uno de los
                      puestos de descenso definidos
                      para{" "}
                      {
                        COMPETITIONS[
                          division
          ...
          <div>
            {buildCareerNews(
              ...
              (n, i) => (
                ...
                >
                  <span className="news-ico text-lg">
                    {n.icon}
                  </span>
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center justify-between text-xs rounded-lg px-2 py-1.5 gap-2 ${
                            mine
                              ? `bg-celeste/10 border border-celeste/30 border-l-[3px] ${
                                  resultColor ||
                                  "border-l-celeste"
                                }`
                              : "hover:bg-white/[0.03]"
...
function CompeticionTab({
  ...
  if (
    ...
    const relegationDetails =
      isSeasonFinished(
        state
      )
        ? firstDivisionRelegationDetails(
            state
          )
        : [];
        ...
        <div className="grid lg:grid-cols-2 gap-3">
          ...
          <div className="hud-panel overflow-hidden">
            ...
            <div className="p-3 space-y-2">
              {relegationDetails.length ===
                0 ? (
                <div className="text-xs text-muted-foreground">
                  Al finalizar la
                  temporada descienden
                  2 equipos: último de
                  la tabla anual y peor
                  promedio.
                </div>
              ) : (
                relegationDetails.map(
                  (r, i) => {
                    const t =
                      getTeamById(
                        r.teamId
          ...
          <div className="hud-panel overflow-hidden lg:col-span-2">
            ...
            <div className="p-3 space-y-2">
              {history.length ===
                0 ? (
                <div className="text-xs text-muted-foreground">
                  La tabla anual de la
                  Temporada 1 quedará
                  guardada al finalizar
                  la temporada.
                </div>
              ) : (
                history.map(
  ...
  if (
    ...
    // Si el mapa persistido no está disponible
    // en saves antiguos, derivamos por las tablas existentes.
    if (
      (
        zoneTables.get(
          "A"
        ...
        <div className="hud-panel p-4">
          ...
          <div className="grid md:grid-cols-2 gap-2 text-xs text-muted-foreground">
            <div className="rounded-lg border border-hud-green/20 bg-hud-green/5 p-3">
              ...
              final a partido único en
              cancha neutral. El ganador
            ...
            <div className="rounded-lg border border-celeste/20 bg-celeste/5 p-3">
              ...
              14 equipos. Se cruzan entre
              zonas; los 7 ganadores se unen
              al perdedor de la final para
              formar 8 equipos de cuartos.
              De ahí salen semis y final por
              el 2° ascenso.
            </div>
          </div>

          <div className="mt-2 text-[10px] text-muted-foreground">
            Todos los cruces del Reducido son
            a partido único. En empate, ventaja
            deportiva para el mejor ubicado.
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="hud-panel p-4 text-xs text-muted-foreground">
          ...
          <div>
            1° Zona A vs 1° Zona B por el
            primer ascenso a Primera C. El
            perdedor entra directamente a
            semifinales del Reducido junto con
            los ganadores de 2°A vs 4°B, 2°B
            vs 4°A y 3°A vs 3°B. El ganador del
            Reducido obtiene el segundo camino
            de ascenso a Primera C.
          </div>
        </div>
      </div>
    );
  ...
  // Regional Amateur: cada grupo va por separado.
  // Nunca se mezclan todos los clubes en una sola tabla.
  if (
    division ===
    "regional_federal_amateur"
  ) {
    ...
    return (
      <div className="space-y-4">
              ...
              <div className="flex items-center justify-between px-1">
                <div>
                  ...
                  <div className="text-[10px] text-muted-foreground uppercase tracking-widest">
                    ...
                    geográficos de 3/4 · ida y vuelta
                  </div>
                </div>
        ...
        <div className="hud-panel p-4 text-xs text-muted-foreground">
          ...
          <div>
            1ª ronda: grupos geográficos de 3/4,
            ida y vuelta. Después: eliminatorias
            regionales a doble partido hasta
            definir 8 campeones regionales. Los 8
            campeones juegan 4 finales nacionales
            a partido único; los 4 ganadores
  ...
  // Federal A: se muestran las cuatro zonas reales de la Fase 1,
  // no una tabla nacional única.
  if (
    division ===
    "federal_a"
  ) {
        ...
        <div className="grid md:grid-cols-2 gap-3">
          <div className="hud-panel p-4">
            ...
            <p className="text-xs text-muted-foreground">
              Entran 5° de la zona de 10,
              4° de cada zona de 9 y el mejor
              5° restante. Se forman 2 zonas de 9
              a una rueda. Los 4 mejores de cada
              una van a cuartos, luego semis y
              final a doble partido por el 1°
              ascenso.
            </p>
          </div>
          ...
          <div className="hud-panel p-4">
            ...
            <p className="text-xs text-muted-foreground">
              Los 19 restantes se dividen en
              grupos 9+10 a una rueda. Los 5
              mejores de cada grupo pasan a
              eliminatorias por el 2° ascenso.
              Los 4 últimos de la Reválida
              descienden al Regional Amateur.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-3">
      ...
      <TablaZona
        ...
        relegated={divisionRelegationCandidates(
          state
        )}
        statusMode="pb"
      />
    ...
    <div className="hud-panel overflow-hidden">
      ...
      {rows.length ===
        ...
        <div className="px-4 py-3 text-xs text-muted-foreground border-b border-border/60">
          El promedio se empieza a
          contar desde la primera temporada
          finalizada de tu carrera.
        </div>
      )}
      ...
      <div className="hud-panel p-4 lg:col-span-2">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              Las stats de simulación se renuevan
              al terminar cada temporada según el
      ...
      <div className="hud-panel p-4 lg:col-span-2">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              Invertí dinero para mejorar el
              crecimiento de stats y los ingresos
              entre temporadas.
            </div>
          </div>
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-2">
          {CLUB_DEVELOPMENT_CATALOG.map(
            opt => {
              const level =
                state.clubDevelopment?.[
                  opt.key
                ] ??
                0;
