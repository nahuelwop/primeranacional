          ...
          <div className="mt-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-2">
              {firstDivisionTeams.map(t => (
                <button
                  key={t.id}
                  onClick={() => startCareer(t.id, "primera_division")}
          ...
          <div className="mt-8 pt-6 border-t border-border/60">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-2">
              {TEAMS.map(t => (
                <button
                  key={t.id}
                  onClick={() => startCareer(t.id, "primera_nacional")}
                  className="p-3 rounded-xl border border-border bg-card/60 hover:bg-secondary hover:scale-[1.03] transition text-left"
                >
                  <Shield team={t} size={48} />
                  <div className="text-xs mt-2 font-display truncate">{t.short}</div>
    ...
    <Shell hideNav>
      ...
      <header className="hud-in grid grid-cols-[auto_1fr_auto] items-center gap-4 border-b border-border/50 pb-0 mb-4">
        <Link to="/" className="flex items-center gap-2.5 pb-3 min-w-0">
          <span className="grid h-9 w-9 place-items-center rounded-lg border border-border bg-card font-display text-sm">
            {first ? "1D" : division === "regional_federal_amateur" ? "RFA" : division === "promocional_amateur" ? "PA" : division === "federal_a" ? "FA" : "PN"}
          </span>
        ...
        <div className="flex items-center gap-3 pb-3 shrink-0">
          ...
          <span className="grid h-8 w-8 place-items-center rounded-full border border-hud-green/60 text-hud-green font-display text-sm tabular-nums">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2.5">
        ...
        <span className="hud-card px-3 py-1.5 flex items-center gap-1.5 text-xs text-hud-green">
          ...
          <span className="font-display tracking-wide">
            {OBJETIVO_LABEL[state.objetivo ?? (first ? "salir_campeon" : "ascenso_directo")]}
          </span>
        </span>
      </div>
          ...
          {recentAch.map(k => {
            ...
            return (
              <div key={k} className="text-sm">
                🎉 ¡Logro desbloqueado! <strong>{a?.icon} {a?.name}</strong>
              </div>
            );
          })}
        </div>
      ...
      <div key={tab} className="mt-3 hud-tab-enter">
        ...
        {tab === "club" && (
          <ClubTab
            ...
            onBuy={onBuyUpgrade}
            onDevelopment={async (k) => {
              const r = buyDevelopment(state, budget, k);
              if (!r.ok) {
                alert(r.error);
                return;
              }
        ...
        {tab === "oficina" && (
          <OficinaTab
            ...
            budget={budget}
            season={season}
            onActivate={onActivateCorruption}
            onAbandon={abandon}
            onSignSponsor={onSignSponsor}
            onCancelSponsor={onCancelSponsor}
          />
        )}

        {tab === "personalizar" && (
          <PersonalizarTab teamId={teamId} />
        )}
        ...
        {tab === "universo" && (
          <CareerFeaturesPanel
            ...
            onSignPlayer={async (player: MarketPlayer) => {
              ...
              const xpState = {
                ...next,
                managerXp: (next.managerXp ?? 0) + 80,
              };

              setState(xpState);
              await persist(xpState, nextBudget, season);
            }}
            onRewardXp={async (amount) => {
              const next = {
                ...state,
                managerXp: (state.managerXp ?? 0) + amount,
              };
...
function InicioTab({
  ...
  const objetivos = division === "primera_division"
    ? [
        {
          label: "Terminar entre los 10 primeros",
          done: pos > 0 && pos <= 10,
        },
        {
          label: "Ganar 8 partidos",
          done: state.matches.filter(
            m =>
              m.played &&
              (
                (m.home === teamId && (m.homeGoals ?? 0) > (m.awayGoals ?? 0)) ||
                (m.away === teamId && (m.awayGoals ?? 0) > (m.homeGoals ?? 0))
              )
          ).length >= 8,
        },
        {
          label: "Mantener la valla invicta en 6 partidos",
          done: state.matches.filter(
            m =>
              m.played &&
              (
                (m.home === teamId && m.awayGoals === 0) ||
                (m.away === teamId && m.homeGoals === 0)
              )
          ).length >= 6,
        },
        {
          label: "Terminar entre los 4 primeros",
          done: pos > 0 && pos <= 4,
        },
      ]
    : [
        {
          label: "Clasificar al Reducido",
          done: pos <= 8,
        },
        {
          label: "Ganar 6 partidos de local",
          done: state.matches.filter(
            m =>
              m.played &&
              m.home === teamId &&
  ...
  const estado = indicators.filter(i =>
    ["moral", "hinchada", "economia"].includes(i.key)
  );
    ...
    <div className="grid lg:grid-cols-[250px_minmax(0,1fr)_340px] gap-3 items-start">
      ...
      <section className="order-1 lg:order-2 space-y-3">
        <div className="hud-in match-hero relative overflow-hidden rounded-[0.9rem] border border-border/50">
          ...
          <div className="relative p-5">
            {nextMatch ? (
              <>
                <div className="text-[11px] uppercase tracking-[0.2em] text-hud-green font-semibold">
                  Próximo partido
                </div>
                ...
                <div className="text-center mt-3">
                  ...
                  <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground mt-1">
                    {COMPETITIONS[division].name}
                    {state.zone ? ` · Zona ${state.zone}` : ""}
                    {nextMatch.isClasico && (
                      <span className="ml-2 text-accent">🔥 Clásico</span>
                    )}
                  </div>
                </div>
                ...
                <div className="mt-5 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-xs text-foreground/90">
                  ...
                  <span className="truncate">
                    🏟️ {(isHome ? myTeam : rivalTeam)?.city ?? "Estadio"}
                  </span>
                </div>
              ...
              <div className="text-center py-6">
                <div className="text-[11px] uppercase tracking-widest text-hud-green">
                  Temporada terminada
                </div>
                ...
                {(() => {
                  ...
                  if (relegated.includes(teamId)) {
                    return (
                      <div className="mt-2 font-display text-lg text-destructive">
                        DESCENDÉS · {down ? COMPETITIONS[down].name.toUpperCase() : "DESCENSO"}
                      </div>
                    );
                  }

                  if (division === "regional_federal_amateur") {
                    if (regionalQualified) {
                      return (
                        <div className="mt-2 font-display text-lg text-hud-green">
                          CLASIFICADO · ELIMINATORIAS REGIONALES
                        </div>
                      );
                    }
                    ...
                    return (
                      <div className="mt-2 font-display text-lg text-hud-green">
                        ASCENDÉS · {COMPETITIONS[next].name.toUpperCase()}
                      </div>
                    );
                  }
                ...
                <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
                  {(
                    (division === "primera_nacional" && pos <= 8) ||
                    (division === "primera_b" && pos >= 2 && pos <= 9) ||
                    (division === "primera_c" && pos <= 7) ||
                    (division === "promocional_amateur" && pos <= 4) ||
                    (division === "regional_federal_amateur" && regionalQualified)
                  ) && (
                    <button
                      onClick={onGoReducido}
                ...
                {(
                  ...
                  <div className="text-[11px] text-muted-foreground mt-3 max-w-xs mx-auto">
                    {division === "regional_federal_amateur"
                      ? "Tu club clasificó a las eliminatorias regionales. Tenés que disputarlas antes de comenzar la nueva temporada."
                      : "Tu club clasificó a una instancia de ascenso. Tenés que disputarla antes de comenzar la nueva temporada."}
                  </div>
                )}

                {division === "primera_division" &&
                  firstDivisionRelegated(state).includes(teamId) && (
                    <div className="text-[11px] text-destructive mt-3 max-w-sm mx-auto">
                      Tu club queda relegado a Primera Nacional para la próxima temporada.
                      En esta división se combinan la tabla anual y los promedios.
                    </div>
                  )}

                {division !== "primera_division" &&
                  divisionRelegationCandidates(state).includes(teamId) && (
                    <div className="text-[11px] text-destructive mt-3 max-w-sm mx-auto">
                      Tu club ocupa uno de los puestos de descenso definidos para{" "}
                      {COMPETITIONS[division].name}.
                    </div>
                  )}
              </div>
      ...
      <aside className="order-3 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="hud-card p-4">
            ...
            <div className="flex items-end gap-2 mt-2">
              <div
                key={pos}
                className="goal-pop font-display text-4xl leading-none tabular-nums"
              >
                {pos || "—"}°
              </div>

              <div className="text-[11px] text-muted-foreground pb-1">
                {row?.pts ?? 0} PTS · {row?.pj ?? 0} PJ
              </div>
            </div>

            <div className="text-[11px] text-muted-foreground mt-2">
              {pos <= 8 ? "Zona de Reducido" : "Fuera del Reducido"}
            </div>
          </div>
        ...
        <div className="hud-card float-soft p-4">
          <div className="text-[11px] uppercase tracking-[0.2em] text-hud-green font-semibold mb-1">
            Noticias
          </div>
        ...
        <div className="hud-card p-4">
          ...
          <div className="space-y-1.5">
            {nextRivals(state, teamId, 4).map(m => {
              const other = careerTeam(
                state,
                m.home === teamId ? m.away : m.home
              );
      ...
      <div className="order-4 lg:col-span-2 lg:col-start-2 grid sm:grid-cols-2 xl:grid-cols-4 gap-3">
        <div className="hud-rise hud-card p-4">
          <div className="text-[11px] uppercase tracking-[0.15em] mb-3">
            Últimos resultados
          </div>
          ...
          <div className="space-y-2">
            {lastResults.map((m, i) => {
              ...
              return (
                ...
                >
                  <span
                    className={`grid h-5 w-5 shrink-0 place-items-center rounded-full text-[10px] font-display ${
                      r === "V"
                        ? "bg-hud-green text-background"
                        : r === "E"
                        ? "bg-muted"
        ...
        <div
          className="hud-rise hud-card p-4"
              ...
              <div key={o.label} className="flex items-start gap-2">
                <span
                  ...
                  className={`relative grid h-4 w-4 shrink-0 place-items-center rounded-full text-[9px] ${
                    o.done
                      ? "bg-hud-green text-background goal-pop goal-flash"
                ...
                <span className={o.done ? "" : "text-muted-foreground"}>
                  {o.label}
                </span>
              </div>
            ))}
            ...
            {estado.map((ind, i) => (
              <div key={ind.key} className="flex items-center gap-2">
                ...
                <span className="stat-bar w-24 shrink-0">
                  <i
                    style={{
                      transform: `scaleX(${Math.max(0.03, ind.value / 100)})`,
...
function TeamBig({ team }: { team?: Team }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2 min-w-0">
      <span className="shield-lit">
        <Shield team={team} size={72} />
      </span>

      <div className="font-display text-lg text-center truncate w-full">
        {team.short}
      </div>
    </div>
  );
    ...
    <div className="hud-panel p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="text-[11px] uppercase tracking-widest text-muted-foreground">
          Calendario · {COMPETITIONS[careerDivision(state, teamId)].name}
          {state.zone ? ` · Zona ${state.zone}` : ""}
        </div>
          ...
          return (
            <div
              ...
              className={`rounded-xl border p-3 transition-colors ${
                isNext
                  ? "border-celeste bg-celeste/[0.06] shadow-[0_0_18px_rgba(56,189,248,0.15)]"
                  : "border-border/60 bg-card/40"
              }`}
            >
              <div className="flex items-center gap-2 mb-2">
                <div className="font-display text-sm tracking-widest text-celeste">
                  FECHA {round}
                </div>

                {isNext && (
                  <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-celeste/20 border border-celeste/50 text-celeste font-display tracking-wide">
                    PRÓXIMA
                  </span>
                )}
              </div>
              ...
              <div className="space-y-1">
                {ms.map(m => {
                  ...
                  return (
                    <div
                      key={m.id}
                      className={`flex items-center justify-between text-xs rounded-lg px-2 py-1.5 gap-2 ${
                        mine
                          ? `bg-celeste/10 border border-celeste/30 border-l-[3px] ${resultColor || "border-l-celeste"}`
                          : "hover:bg-white/[0.03]"
                      }`}
                    >
...
function CompeticionTab({
  ...
  if (division === "primera_division") {
    ...
    const relegated = isSeasonFinished(state)
      ? firstDivisionRelegated(state)
      : [];

    const relegationDetails = isSeasonFinished(state)
      ? firstDivisionRelegationDetails(state)
      : [];

    return (
      <div className="space-y-3">
        <CompetitionMovementPanel
          division={division}
          state={state}
          teamId={teamId}
        />
        ...
        <TablaZona
          title={`Tabla anual · Temporada ${
            history.length
              ? Math.max(...history.map(h => h.season), 0)
              : seasonLabel(state)
          }`}
          rows={annual}
          highlight={teamId}
        ...
        <div className="grid lg:grid-cols-2 gap-3">
          ...
          <div className="hud-panel overflow-hidden">
            <div className="px-4 py-2.5 font-display text-sm uppercase tracking-widest text-celeste border-b border-border">
              Descensos · Primera División
            </div>

            <div className="p-3 space-y-2">
              {relegationDetails.length === 0 ? (
                <div className="text-xs text-muted-foreground">
                  Al finalizar la temporada descienden 2 equipos:
                  último de la tabla anual y peor promedio.
                </div>
              ) : (
                relegationDetails.map((r, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="ml-auto text-destructive/80 text-right">
                        {r.reason}
                      </span>
                    </div>
                  );
          ...
          <div className="hud-panel overflow-hidden lg:col-span-2">
            ...
            <div className="p-3 space-y-2">
              {history.length === 0 ? (
                <div className="text-xs text-muted-foreground">
                  La tabla anual de la Temporada 1 quedará guardada al finalizar la temporada.
                </div>
              ) : (
                history.map(h => {
                  ...
                  return (
                    <div
                      key={`${h.division}-${h.season}`}
                      className="rounded-lg border border-border/60 bg-card/40 px-3 py-2 flex items-center justify-between text-xs"
                    >
  ...
  if (division === "primera_nacional") {
    ...
    const zm =
      state.federalZoneMap ??
      Object.fromEntries(
        getTeamsByDivision(division).map(t => [
          t.id,
          t.zone ?? "A",
        ])
      );
    ...
    for (const z of ["A", "B"]) {
      ...
      for (const m of allMatches) {
        if (
          ...
          rows = applyMatchToStandings(rows, m);
        }
      }
      ...
      for (const m of allMatches) {
        if (
          m.played &&
          ids.includes(m.home) !== ids.includes(m.away)
        ) {
          rows = applyMatchToStandings(rows, m);
        }
      }
    ...
    if (
      (zoneTables.get("A")?.length ?? 0) === 0 ||
      (zoneTables.get("B")?.length ?? 0) === 0
    ) {
      zoneTables.set(
        state.zone || "A",
        sortStandings(state.standings)
      );

      if (state.otherStandings) {
        zoneTables.set(
          state.zone === "A" ? "B" : "A",
          sortStandings(state.otherStandings)
        );
      }
    }
    ...
    const labels = (z: string) =>
      `${z === "A" ? "Zona A" : "Zona B"} · 1° = Final por 1er ascenso · 2°-8° = Reducido`;
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="hud-panel p-4">
          <div className="font-display text-sm uppercase tracking-widest text-celeste mb-2">
            ASCENSO Y REDUCIDO · PRIMERA NACIONAL
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-2 text-xs text-muted-foreground">
            <div className="rounded-lg border border-hud-green/20 bg-hud-green/5 p-3">
              <b className="text-hud-green">
                1° de Zona A + 1° de Zona B:
              </b>{" "}
              final a partido único en cancha neutral.
              El ganador asciende a Primera División.
            </div>

            <div className="rounded-lg border border-celeste/20 bg-celeste/5 p-3">
              <b className="text-celeste">
                2° al 8° de A + 2° al 8° de B:
              </b>{" "}
              14 equipos. Se cruzan entre zonas; los
              7 ganadores se unen al perdedor de la
              final para formar 8 equipos de cuartos.
              De ahí salen semis y final por el 2° ascenso.
            </div>
          </div>

          <div className="mt-2 text-[10px] text-muted-foreground">
            Todos los cruces del Reducido son a partido único.
            En empate, ventaja deportiva para el mejor ubicado.
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="grid lg:grid-cols-2 gap-3">
          {tables.map(({ z, rows }) => (
            <TablaZona
              key={z}
              title={`Zona ${z} · ${
                z === "A" ? "8 equipos" : "9 equipos"
              } · 1° = final · 2°-4° = Reducido`}
              rows={rows}
              highlight={teamId}
              matches={allMatches}
              division={division}
        ...
        <div className="hud-panel p-4 text-xs text-muted-foreground">
          <div className="font-display text-sm uppercase tracking-widest text-celeste mb-2">
            PROMOCIONAL AMATEUR · ASCENSO
          </div>

          <div>
            1° Zona A vs 1° Zona B por el primer ascenso a Primera C.
            El perdedor entra directamente a semifinales del Reducido
            junto con los ganadores de 2°A vs 4°B, 2°B vs 4°A y
            3°A vs 3°B. El ganador del Reducido obtiene el segundo
            camino de ascenso a Primera C.
          </div>
        </div>
      </div>
    );
  ...
  if (division === "regional_federal_amateur") {
    const roster =
      state.leagueRosters?.regional_federal_amateur ??
      getTeamsByDivision(division).map(t => t.id);
    ...
    for (const id of roster) {
      ...
      (groups.get(key) ??
        groups.set(key, []).get(key)!).push(id);
    }
    ...
    for (const t of tables) {
      (byRegion.get(t.region) ??
        byRegion.set(t.region, []).get(t.region)!).push(t);
    }
    ...
    return (
      <div className="space-y-4">
            ...
            <div className="flex items-center justify-between px-1">
              <div>
                ...
                <div className="text-[10px] text-muted-foreground uppercase tracking-widest">
                  Primera ronda · grupos geográficos de 3/4 · ida y vuelta
                </div>
              </div>
        ...
        <div className="hud-panel p-4 text-xs text-muted-foreground">
          <div className="font-display text-sm uppercase tracking-widest text-celeste mb-2">
            FORMATO REGIONAL FEDERAL AMATEUR
          </div>

          <div>
            1ª ronda: grupos geográficos de 3/4, ida y vuelta.
            Después: eliminatorias regionales a doble partido hasta
            definir 8 campeones regionales. Los 8 campeones juegan
            4 finales nacionales a partido único; los 4 ganadores
            ascienden al Federal A.
          </div>
        </div>
      </div>
    );
  ...
  if (division === "federal_a") {
    const ids =
      state.leagueRosters?.federal_a ??
      getTeamsByDivision(division).map(t => t.id);

    const zm =
      state.federalZoneMap ??
      buildFederalZoneMap(ids);
    ...
    const zones = ["A", "B", "C", "D"].map(z => {
      ...
      for (const m of allMatches) {
        if (
          m.played &&
          zoneIds.includes(m.home) &&
          zoneIds.includes(m.away)
        ) {
          rows = applyMatchToStandings(rows, m);
        }
      }
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="grid lg:grid-cols-2 gap-3">
          {zones.map(({ z, rows, count }) => (
            <TablaZona
              key={z}
              title={`Fase 1 · Zona ${z} (${count} equipos)`}
              rows={rows}
              highlight={teamId}
              matches={allMatches}
              division={division}
              statusMode="fa_phase1"
            />
          ))}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-3">
          <div className="hud-panel p-4">
            <div className="font-display text-sm uppercase tracking-widest text-hud-green mb-2">
              ZONA CAMPEONATO · 18
            </div>

            <p className="text-xs text-muted-foreground">
              Entran 5° de la zona de 10, 4° de cada zona de 9 y el
              mejor 5° restante. Se forman 2 zonas de 9 a una rueda.
              Los 4 mejores de cada una van a cuartos, luego semis y
              final a doble partido por el 1° ascenso.
            </p>
          </div>
          ...
          <div className="hud-panel p-4">
            ...
            <p className="text-xs text-muted-foreground">
              Los 19 restantes se dividen en grupos 9+10 a una rueda.
              Los 5 mejores de cada grupo pasan a eliminatorias por el
              2° ascenso. Los 4 últimos de la Reválida descienden al
              Regional Amateur.
            </p>
          </div>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid lg:grid-cols-2 gap-3">
          {tables.map(({ z, rows }) => (
            <TablaZona
              key={z}
              title={`Zona ${z} · 1° = Final · 2°-7° = Reducido`}
              rows={rows}
              highlight={teamId}
              matches={allMatches}
              division={division}
...
function CompetitionMovementPanel({
  division,
  state,
  teamId,
}: {
  ...
  const relegationLabel = (() => {
    if (division === "primera_division") {
      return "2 descensos: 1 por último de la tabla anual y 1 por peor promedio. Si coincide, la plaza anual pasa al 29°.";
    }

    if (division === "primera_nacional") {
      return "4 descensos: 2 clubes metropolitanos a Primera B y 2 clubes del interior a Federal A.";
    }

    if (division === "primera_b") {
      return "2 últimos de la tabla general descienden a Primera C.";
    }

    if (division === "primera_c") {
      return "1 descenso: último de la tabla general → Promocional Amateur.";
    }

    if (division === "promocional_amateur") {
      return "Sin descenso modelado; dos caminos de ascenso a Primera C.";
    }

    if (division === "regional_federal_amateur") {
      return "No hay descenso: última categoría jugable del circuito federal.";
    }

    if (division === "federal_a") {
      return "4 descensos: los últimos 4 de la Fase Reválida → Regional Federal Amateur.";
    }

    if (
      rules.relegation.length === 0 ||
      rules.relegation.every(r => r.slots === 0)
    ) {
      return "No hay descenso modelado en esta categoría.";
    }

    return `Descenso: ${rules.relegation.reduce(
      (sum, r) => sum + r.slots,
      0
    )} puestos por tabla.`;
  })();
  ...
  const formatLabel =
    rules.formatLabel ??
    (rules.hasZones
      ? `Zonas: ${rules.zones.join(" · ")}`
      : "Tabla general");

  const promotionLabel =
    rules.promotion.length === 0
      ? "No hay ascenso porque esta es la máxima categoría."
      : `${rules.promotion[0].directSlots} ascenso(s) directo(s)${
          rules.promotion[0].playoffSlots
            ? ` + ${rules.promotion[0].playoffSlots} por playoff/reducido`
            : ""
        } a ${COMPETITIONS[rules.promotion[0].to].name}.`;
  ...
  return (
    <div className="hud-panel p-4">
      <div className="flex items-center justify-between gap-3 mb-3">
        <div>
          ...
          <div className="text-[11px] text-muted-foreground">
            {rules.name} · puesto actual {position || "—"}°
          </div>
        </div>
        ...
        {state && isSeasonFinished(state) && (
          <span
            className={`text-[10px] uppercase tracking-widest font-display ${
              relegated.includes(teamId)
                ? "text-destructive"
                : promoted.includes(teamId)
                ? "text-hud-green"
      ...
      <div className="rounded-lg border border-border/60 bg-card/30 p-3 mb-2">
        <div className="text-[10px] uppercase tracking-widest text-celeste mb-1">
          Formato
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-2">
        <div className="rounded-lg border border-hud-green/20 bg-hud-green/5 p-3">
          <div className="text-[10px] uppercase tracking-widest text-hud-green mb-1">
            Ascenso
          </div>
          <div className="text-xs text-foreground/90">
            {promotionLabel}
...
function TablaPromedios({
  ...
  rows: ReturnType<typeof buildAverageTable>;
  highlight?: string;
}) {
  return (
    <div className="hud-panel overflow-hidden">
      <div className="px-4 py-2.5 font-display text-sm uppercase tracking-widest text-celeste border-b border-border flex items-center justify-between">
        <span>Promedios · Primera División</span>
      ...
      {rows.length === 0 && (
        <div className="px-4 py-3 text-xs text-muted-foreground border-b border-border/60">
          El promedio se empieza a contar desde la primera temporada finalizada de tu carrera.
        </div>
      )}
            ...
            {rows.map((r, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="text-center">{r.seasons}</td>
                  <td className="text-center tabular-nums">{r.pj}</td>
                  <td className="text-center tabular-nums">{r.pts}</td>

                  <td className="text-center tabular-nums font-display text-celeste">
                    {r.avgPtsPerMatch.toFixed(2)}
                  </td>
                </tr>
              );
            })}
    ...
    <div className="hud-panel overflow-hidden">
          ...
          <tbody>
            {rows.map((r, i) => {
              ...
              if (statusMode === "pn") {
                ...
                const directSlots =
                  rules.promotion[0]?.directSlots ?? 0;

                const playoffSlots =
                  rules.promotion[0]?.playoffSlots ?? 0;

                const playoffRange =
                  directSlots + playoffSlots * 8;
              ...
              return (
                <tr
                  ...
                  className={`border-t border-border/40 transition-colors ${
                    ...
                    isRelegated
                      ...
                      : inReducido
                      ? "border-l-[3px] border-l-celeste/70"
                ...
                >
                  ...
                  <td className="text-center tabular-nums font-display text-base">
                    {r.pts}
                  </td>
                  ...
                  <td className="px-2 py-1.5 hidden sm:table-cell">
                    <div className="flex gap-0.5 justify-center">
                      ...
                      {form.map((f, fi) => (
                        <span
                          key={fi}
                          className={`w-3.5 h-3.5 rounded-[3px] grid place-items-center text-[8px] font-bold ${
                            f === "V"
                              ? "bg-hud-green/80 text-black"
                              : f === "D"
                              ? "bg-destructive/80 text-white"
                              : "bg-muted-foreground/50 text-black"
                          }`}
                        >
                          {f}
                        </span>
                  ...
                  <td className="px-2 py-1.5 text-center text-[10px] font-display">
                    {isRelegated ? (
      ...
      <div className="px-4 py-2 text-[10px] text-muted-foreground border-t border-border/40">
        {division === "primera_division"
          ...
          : COMPETITIONS[division].relegation.every(
              r => r.slots === 0
            )
          ? "Sin descenso modelado en esta categoría"
          : `Ascenso directo: ${
              COMPETITIONS[division].promotion[0]
                ?.directSlots ?? 0
            } · Descenso: ${COMPETITIONS[division].relegation.reduce(
              (sum, r) => sum + r.slots,
              0
            )} puestos`}
      </div>
...
function ClubTab({
  ...
  unlocked: number;
  onBuy: (
    k: typeof STADIUM_UPGRADE_CATALOG[number]["key"]
  ) => void;
  onDevelopment: (
    k: keyof ClubDevelopment
  ) => void;
}) {
  return (
    <div className="grid lg:grid-cols-2 gap-3">
      <div className="hud-panel p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-xs text-muted-foreground">
            Ingresos ×{incomeMultiplier(state).toFixed(2)}
          </div>
        </div>
        ...
        <div className="space-y-2">
          {STADIUM_UPGRADE_CATALOG.map(opt => {
            ...
            return (
              <div
                key={opt.key}
                className="flex items-center gap-2 border border-border/60 rounded-xl p-2.5 bg-card/40"
              >
                ...
                {owned ? (
                  <span className="text-xs px-2 py-1 rounded bg-celeste/20 text-celeste font-display shrink-0">
                    ACTIVA
                  </span>
                ) : (
                  <button
      ...
      <div className="hud-panel p-4 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          <Stat
            label="Goles totales"
            value={state.totalGoalsScored}
          />

          <Stat
            label="Mejor invicto"
            value={state.bestUnbeaten}
          />
          ...
          {state.zoneChampions.length === 0 ? (
            ...
            <div className="space-y-1">
              {state.zoneChampions.map((z, i) => (
                <div
                  key={i}
                  className="text-xs bg-card/50 rounded-lg px-2 py-1.5 border border-border/50"
                >
                  T{z.season} · Zona {z.zone} ·{" "}
                  <span className="font-display">
                    {getTeamById(z.teamId)?.short}
                  </span>
                </div>
              ))}
            </div>
        ...
        <Link
          to="/equipos/$id"
          params={{ id: teamId }}
          className="inline-block text-sm text-celeste underline"
        >
          Ver ficha, plantel y estadio →
        </Link>
      </div>
      ...
      <div className="hud-panel p-4 lg:col-span-2">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              Las stats de simulación se renuevan al terminar cada temporada según el rendimiento del equipo.
            </div>
          </div>
        ...
        {(() => {
          const r =
            state.teamRatings?.[teamId] ??
            getTeamById(teamId)?.stats;
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[
                ...
                >
                  ...
                  <div className="font-display text-2xl text-celeste">
                    {value}
                  </div>
                </div>
              ))}
      ...
      <div className="hud-panel p-4 lg:col-span-2">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              Invertí dinero para mejorar el crecimiento de stats y los ingresos entre temporadas.
            </div>
          </div>

          <div className="text-xs text-celeste font-display">
            Caja ${budget}
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-2">
          {CLUB_DEVELOPMENT_CATALOG.map(opt => {
            const level =
              state.clubDevelopment?.[opt.key] ?? 0;

            const maxed =
              level >= opt.costs.length;
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="text-[10px] text-celeste">
                    NIVEL {level}/{opt.costs.length}
                  </div>
                </div>

                <div className="text-[11px] text-muted-foreground mt-1 min-h-[32px]">
                  {opt.desc}
                </div>
                ...
                <button
                  disabled={maxed || budget < cost}
                  onClick={() => onDevelopment(opt.key)}
                  className="w-full mt-3 py-2 rounded-lg bg-celeste text-primary-foreground font-display text-xs disabled:opacity-35"
                >
                  {maxed
                    ? "MAX"
                    : `INVERTIR $${cost}`}
...
function OficinaTab({
  ...
  season: number;
  onActivate: (
    k: typeof CORRUPTION_CATALOG[number]["kind"]
  ) => void;
  onAbandon: () => void;
  onSignSponsor: (
    d: SponsorDeal
  ...
  const active =
    state.activeCorruption &&
    state.activeCorruption.matchesLeft > 0
      ? state.activeCorruption
      : null;
  ...
  return (
    <div className="space-y-4">
      <SponsorsPanel
        budget={budget}
        season={season}
        deal={state.sponsor ?? null}
        onSign={onSignSponsor}
        onCancel={onCancelSponsor}
      />
      ...
      <div className="grid lg:grid-cols-[minmax(0,1fr)_280px] gap-3">
          ...
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            ...
            {active ? (
              <div className="text-xs text-accent">
                Activo:{" "}
                {CORRUPTION_CATALOG.find(
                  o => o.kind === active.kind
                )?.name}{" "}
                · {active.matchesLeft} fechas
              </div>
            ) : (
              <div className="text-xs text-muted-foreground">
                Ninguno activo
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-display">
                    ...
                    <span className="text-xs text-muted-foreground">
                      · {opt.matches} fechas · -{opt.penaltyPct}% ingresos
                    </span>
                  </div>
        ...
        <div className="hud-panel p-4 space-y-3">
          ...
          <div className="text-sm">
            Caja:{" "}
            <span className="font-display text-celeste text-xl">
              ${budget}
            </span>
          </div>
          ...
          <div className="text-xs text-muted-foreground">
            ...
            {incomeMultiplier(state).toFixed(2)}
          </div>

          {state.incomePenalty && (
            <div className="text-xs text-destructive">
              Penalidad -{state.incomePenalty.pct}% por{" "}
              {state.incomePenalty.matchesLeft} fechas.
            </div>
          )}
...
function PersonalizarTab({
  ...
  const kits = [
    {
      name: "Titular",
      primary: t.primary,
      secondary: t.secondary ?? "#111",
    },
    {
      name: "Alternativa",
      primary: t.secondary ?? "#111",
      secondary: t.primary,
    },
  ];
  ...
  const toggleTrack = (id: string) => {
    ...
    const disabled = music.prefs.disabled_track_ids.includes(id)
      ? music.prefs.disabled_track_ids.filter(
          x => x !== id
        )
      : [
          ...music.prefs.disabled_track_ids,
          id,
        ];

    music.setPrefs({
      ...music.prefs,
      disabled_track_ids: disabled,
    });
  };
  ...
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
      {kits.map((k, i) => (
        <div
          key={k.name}
          className="hud-rise hud-card p-5 text-center"
          style={{
        ...
        >
          ...
          <div
            className="mx-auto h-28 w-24 rounded-lg border border-border"
            style={{
              background: `linear-gradient(90deg, ${k.primary} 0 33%, ${k.secondary} 33% 66%, ${k.primary} 66% 100%)`,
            }}
          />
      ...
      <div
        className="hud-rise hud-card p-5"
        style={{
      ...
      >
        ...
        <Link
          to="/equipos/$id"
          params={{ id: teamId }}
          className="mt-4 inline-block text-sm text-hud-green"
        >
          Ver ficha del club →
        </Link>
      </div>
      ...
      <div
        className="hud-rise hud-card p-5 sm:col-span-2 lg:col-span-3"
        style={{
      ...
      >
        <div className="flex items-center justify-between mb-3">
          ...
          {music && (
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              ...
              <input
                type="checkbox"
                checked={music.prefs.music_enabled}
                onChange={e =>
                  music.setPrefs({
                    ...music.prefs,
        ...
        {!music || music.tracks.length === 0 ? (
          <div className="text-xs text-muted-foreground">
            Todavía no hay canciones cargadas por el admin.
          </div>
        ) : (
          <div
          ...
          >
            {music.tracks.map(track => {
              const disabled =
                music.prefs.disabled_track_ids.includes(
                  track.id
                );
              ...
              return (
                <label
                  key={track.id}
                  className="flex items-center gap-3 rounded-lg border border-border p-2 cursor-pointer hover:bg-secondary/40 transition"
                >
                  {track.cover_url ? (
                    <img
                      src={track.cover_url}
                    ...
                    <div className="w-9 h-9 rounded bg-secondary grid place-items-center shrink-0 text-sm">
                      🎵
                    </div>
                  )}
...
function Shell({
  ...
  // Música de fondo: solo suena en el menú de Carrera.
  // Intro y partidos la pausan.
  const music = useCareerMusic(musicActive);

  return (
    <CareerMusicContext.Provider value={music}>
      <div
        className="relative min-h-screen flex flex-col hud-shell"
        data-sfx-root
      >
        <AmbientStadium />
        ...
        <div className="relative z-10 flex flex-1 flex-col">
          ...
          <main className="flex-1 w-full max-w-[1400px] mx-auto px-3 sm:px-6 py-5 hud-boot">
            {children}
          </main>
        </div>
