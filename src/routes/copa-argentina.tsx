  ...
  head: () => ({
    meta: [
      { title: "Copa Argentina · Primera Heads" },
      {
        name: "description",
        content:
          "Eliminación directa a partido único, cancha neutral. 64 equipos, un campeón.",
      },
    ],
  }),
...
function loadOrBuild(
  ...
  try {
    ...
    /* ignore broken local state and rebuild */
  }
...
function CopaArgentinaPage() {
  ...
  useEffect(() => {
    ...
    // La intro es por inicio de Copa y por equipo.
    // No se marca como vista globalmente por temporada:
    // al elegir otro club vuelve a aparecer.
    setShowIntro(true);
  }, [teamId, season, mode]);
  ...
  useEffect(() => {
    ...
    fetchGameSettings()
      .then((settings) => {
        if (active) {
          setIntroVideo(
            settings.intro_videos?.copa_argentina ?? null
          );
        }
      })
      .catch(() => {
  ...
  const nextMatch = state
    ? nextCopaMatchForUser(state)
    : null;

  const home = nextMatch
    ? TEAMS_BY_ID[nextMatch.home]
    : undefined;

  const away = nextMatch
    ? TEAMS_BY_ID[nextMatch.away]
    : undefined;
        ...
        <main className="relative min-h-[calc(100vh-65px)] overflow-hidden">
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-4 py-8 md:py-12">
            <div className="text-center max-w-3xl mx-auto">
              <div className="text-[11px] uppercase tracking-[0.4em] text-celeste mb-3">
              ...
              <p className="mt-3 text-white/60">
                Elegí cualquier club habilitado, incluso sin iniciar
                sesión. Esta Copa queda separada de tu Modo Carrera.
              </p>

              <div className="mt-5 flex flex-wrap justify-center gap-2 text-[10px] uppercase tracking-widest text-white/50">
                <span className="hud-card px-3 py-2 rounded-full">
                  64 CLUBES
                </span>
              ...
              {selectableTeams.map((group) => (
                <section key={group.id}>
                  <div className="flex items-end justify-between gap-4 mb-3">
                    <div>
                      <div className="text-[10px] uppercase tracking-[0.3em] text-celeste/80">
                        Participantes
                      </div>

                      <h2 className="font-display text-2xl md:text-3xl tracking-widest">
                        {group.label}
                      </h2>
                    </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
                    {group.teams.map((team) => (
                      ...
                      >
                        ...
                        <div className="mt-2 text-[9px] uppercase tracking-[0.15em] text-celeste/0 group-hover:text-celeste transition-colors">
                          Elegir club →
                        </div>
                      </button>
                    ))}
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 max-w-5xl w-full mx-auto px-4 py-6">
          <div className="text-center mb-3">
            <div className="text-[11px] uppercase tracking-[0.3em] text-celeste">
              {COPA_ROUND_LABEL[nextMatch.round]} · Cancha neutral
            </div>
          </div>
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 py-6">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
          <div>
            <div className="text-[10px] uppercase tracking-[0.35em] text-celeste">
              {mode === "career"
                ? "MODO CARRERA · COPA ARGENTINA"
                : "TORNEO INDEPENDIENTE"}
            </div>

            <div className="font-display text-3xl md:text-4xl tracking-widest">
              COPA ARGENTINA
            </div>

            <div className="text-xs text-muted-foreground mt-1">
              {selectedTeam.name} · Temporada {season}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="px-4 py-2 rounded-lg border border-white/15 hover:border-celeste/60 hover:bg-celeste/5 text-xs font-display tracking-widest"
            >
              VER INTRO
            </button>
        ...
        {finished ? (
          <div className="hud-panel p-6 text-center">
            {state.champion === state.userTeamId ? (
              <div className="font-display text-2xl text-hud-green">
            ...
            {mode === "standalone" ? (
              <button
                onClick={startNewStandalone}
                className="mt-5 px-5 py-2.5 rounded-lg bg-celeste text-primary-foreground font-display tracking-widest"
              >
                ELEGIR OTRO EQUIPO
              </button>
            ) : (
              <Link
                to="/carrera"
                className="inline-block mt-5 px-5 py-2.5 rounded-lg bg-celeste text-primary-foreground font-display tracking-widest"
              >
                VOLVER A CARRERA
              </Link>
            )}
          ...
          <div className="hud-panel p-6 text-center mb-6">
            <div className="text-[11px] uppercase tracking-[0.25em] text-celeste mb-3">
              {COPA_ROUND_LABEL[nextMatch.round]}
            </div>

            <div className="text-[10px] uppercase tracking-[0.25em] text-white/40 mb-5">
              {selectedTeam.name} · camino al título
            </div>
            ...
            <div className="mt-3 text-[10px] uppercase tracking-[0.2em] text-white/35">
              Sin cuenta · progreso guardado en este dispositivo
            </div>
          </div>
        ) : (
  ...
  return (
    <section className="copa-bracket-section">
      <div className="copa-bracket-heading">
        <div>
          <div className="copa-bracket-kicker">
            CAMINO AL TÍTULO
          </div>

          <h2 className="copa-bracket-title">
        ...
        <div className="copa-bracket-track">
          {COPA_ROUND_ORDER.map(
            (round, roundIndex) => {
              ...
              return (
                <div
                  className={`copa-round copa-round-${roundIndex + 1}`}
                  key={round}
                >
                  <div className="copa-round-header">
                    <div className="copa-round-number">
                      FASE{" "}
                      {String(
                        roundIndex + 1
                      ).padStart(2, "0")}
                    ...
                    <div className="copa-round-title">
                      {COPA_ROUND_LABEL[round]}
                    </div>
                  </div>
                          ...
                          >
                            ...
                            >
                              <div className="copa-team-info">
                                ...
                                <span className="copa-team-name">
                                  {h?.short ??
                                    "POR DEFINIR"}
                                </span>
                              </div>

                              <span className="copa-team-score">
                                {m.played
                                  ? m.homeGoals
                                  : "·"}
                              </span>
      ...
      <div className="copa-bracket-legend">
        <div>
          <span className="copa-legend-swatch copa-legend-user" />
          Tu camino
        </div>

        <div>
          <span className="copa-legend-swatch copa-legend-played" />
          Partido jugado
        </div>

        <div>
          <span className="copa-legend-swatch copa-legend-pending" />
          Por jugar
        </div>
      </div>
    </section>
