...
import { type Team, type Narrator } from "@/data/teams";
...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [{ title: "Admin · Primera Heads" }],
  }),
  component: AdminPage,
});
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Gestioná equipos y configuración global del juego.
            </p>
          </div>
        ...
        <div className="flex gap-2 border-b border-border mb-6 overflow-x-auto">
          {(
            ...
            <button
              key={t.k}
              onClick={() => setTab(t.k)}
              className={`px-4 py-2 font-display text-sm tracking-wider border-b-2 transition whitespace-nowrap ${
                tab === t.k
...
const NARRATOR_FIELDS: {
  ...
  {
    field: "penal_goal_urls",
    label: "Penal → Gol",
    sub: "penal-gol",
  },
  {
    field: "penal_save_urls",
    label: "Penal → Atajada",
    sub: "penal-atajada",
  },
  {
    field: "penal_decisive_urls",
    label: "Penal → Decisivo",
    sub: "penal-decisivo",
  },
  {
    field: "clasico_previa_urls",
    label: "Previa del Clásico",
    sub: "clasico-previa",
  },
];
...
function RelatoresTab() {
  ...
  async function load() {
    ...
    if (error) {
      ...
      setNarrators((data ?? []) as GlobalNarrator[]);
    }
  ...
  async function deleteNarrator(id: string) {
    if (
      !confirm(
        "¿Eliminar este relator y todos sus audios de la lista?"
      )
    ) {
      return;
    }
  ...
  async function uploadAudios(
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const path = `global-relatores/${id}/${sub}/${Date.now()}-${Math.random()
          .toString(36)
          .slice(2, 8)}.${ext}`;
        ...
        if (sErr || !data) {
          throw sErr ?? new Error("No se pudo firmar el audio");
        }
      ...
      const newUrls = [
        ...(((target?.[field] as string[] | undefined) ?? [])),
        ...urls,
      ];
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <p className="text-sm text-muted-foreground max-w-xl">
          Relatores globales: se ofrecen en cualquier partido, sin importar
          qué equipos jueguen.
        </p>
      ...
      {narrators.length === 0 && (
        <div className="text-sm text-muted-foreground border border-dashed border-border rounded-lg p-6 text-center">
          Todavía no hay relatores globales.
        </div>
      )}
          ...
          >
            <div className="flex items-center gap-2">
              <Input
                value={n.name}
                onChange={(e) =>
                  renameNarrator(n.id, e.target.value)
                }
...
function MusicaTab() {
  ...
  async function load() {
    ...
    const { data, error } = await (
      supabase.from("career_music_tracks" as any) as any
    )
      .select("*")
      .order("sort_order", { ascending: true });
        ...
        {tracks.map((tr) => (
          ...
          >
            {tr.cover_url ? (
              ...
              <div className="w-14 h-14 rounded bg-muted grid place-items-center shrink-0 text-xs">
                sin portada
              </div>
            )}
  ...
  return (
    <div>
      <div className="flex items-center gap-2 flex-wrap mb-3">
        {DIVISION_ORDER.map((d) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-display tracking-wide transition ${
              d === division
                ? "bg-celeste text-primary-foreground"
      ...
      <div className="mt-4 text-xs text-muted-foreground">
        {COMPETITIONS[division].name} · {list.length} equipos
      </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-muted-foreground">
                {division === "regional_federal_amateur"
                  ? `Región ${t.regionalRegion ?? "—"} · Grupo ${
                      t.regionalGroup ?? "—"
                    }`
                  : division === "federal_a"
                  ? `Zona ${t.zone ?? "—"}`
                  : COMPETITIONS[division].hasZones
                  ? `Zona ${t.zone ?? "—"} · `
                  : ""}
                {division !== "regional_federal_amateur" &&
                  t.city}
              ...
              <div className="text-[10px] text-muted-foreground">
                VEL {t.stats.speed} · SAL {t.stats.jump} · POT{" "}
                {t.stats.power} · DEF {t.stats.defense}
              </div>
            </div>
          </button>
        ))}
      ...
      {list.length === 0 && (
        <div className="border border-dashed border-border rounded-xl p-8 mt-4 text-center text-sm text-muted-foreground">
          No se encontraron equipos en esta división.
        </div>
      )}
...
const COIMAS_LABELS: {
  ...
  {
    key: "clasificar_reducido",
    label: "Clasificar al Reducido",
  },
  { key: "forzar_ascensos", label: "Forzar ascenso" },
  {
    key: "forzar_descensos",
    label: "Forzar descenso rival",
  },
  { key: "anular_goles", label: "Anular goles rivales" },
  { key: "arbitro_amigo", label: "Árbitro amigo" },
  {
    key: "penal_inventado",
    label: "Penal inventado",
  },
  {
    key: "expulsar_rival",
    label: "Expulsar rival",
  },
  { key: "doping", label: "Doping (boost físico)" },
  {
    key: "hinchada_comprada",
    label: "Hinchada comprada",
  },
  {
    key: "sponsor_fantasma",
    label: "Sponsor fantasma (+$)",
  },
  { key: "gol_fantasma", label: "Gol fantasma" },
  { key: "var_apagado", label: "VAR apagado" },
  {
    key: "amarilla_rival",
    label: "Amarilla al rival",
  },
  {
    key: "cambio_fixture",
    label: "Cambiar fixture",
  },
  {
    key: "descontar_puntos_rival",
    label: "Descontar puntos rival",
  },
  {
    key: "bonus_presupuesto",
    label: "Bonus presupuesto",
  },
];
 ...
 * IMPORTANTE:
 *
 * Todas las intros se guardan en:
 *
 * settings.intro_videos
 *
 * con estas claves:
 *
 * primera_division
 ...
 * copa_argentina
 *
 * La compatibilidad con intro_video_url se mantiene.
 */
...
const INTRO_OPTIONS: {
...
}[] = [
  {
    ...
    description:
      "Intro que se reproduce al comenzar una carrera en Primera División.",
  },
  {
    key: "primera_nacional",
    label: "Primera Nacional",
    description:
      "Intro específica de la Primera Nacional.",
  },
  {
    key: "primera_b",
    label: "Primera B Metropolitana",
    description:
      "Intro específica de la Primera B Metropolitana.",
  },
  {
    key: "primera_c",
    label: "Primera C",
    description:
      "Intro específica de la Primera C.",
  },
  {
    key: "promocional_amateur",
    label: "Promocional Amateur",
    description:
      "Intro específica del Promocional Amateur.",
  },
  {
    key: "federal_a",
    label: "Torneo Federal A",
    description:
      "Intro específica del Federal A.",
  },
  {
    key: "regional_federal_amateur",
    label: "Regional Federal Amateur",
    description:
      "Intro específica del Regional Federal Amateur.",
  },
  {
    key: "copa_argentina",
    label: "Copa Argentina",
    description:
      "Intro independiente de la Copa Argentina.",
  },
];
...
function AjustesTab() {
    ...
    fetchGameSettings()
      .then((s) => {
        const normalized = {
          ...
          ...s,
          intro_videos: {
            ...(DEFAULT_SETTINGS as any).intro_videos,
            ...((s as any).intro_videos ?? {}),
          },
        } as GameSettings;

        /*
         * Compatibilidad:
         * si todavía existe la intro vieja,
         * la mostramos también como Primera Nacional.
         */
        const introVideos = {
          ...(((normalized as any).intro_videos ??
            {}) as Record<string, string | null>),
        };

        if (
          !introVideos.primera_nacional &&
          normalized.intro_video_url
        ) {
          introVideos.primera_nacional =
            normalized.intro_video_url;
        }

        normalized.intro_videos =
          introVideos as any;
  ...
  function getIntroVideo(key: string) {
    const videos =
      ((settings as any).intro_videos ??
        {}) as Record<string, string | null>;
  ...
  function toggleFlag(key: keyof CoimasFlags) {
    setSettings((s) => ({
      ...
      coimas_flags: {
        ...s.coimas_flags,
        [key]: !s.coimas_flags[key],
      },
    }));
  }

  async function uploadIntroFile(
    key: string,
    file: File
  ) {
    setBusy(true);
    ...
    try {
      if (!file.type.startsWith("video/")) {
        throw new Error(
          "El archivo tiene que ser un video."
        );
      }

      if (file.size > 250 * 1024 * 1024) {
        throw new Error(
          "El video no puede superar 250 MB."
        );
      }
      ...
      const path = `intro/${safeKey}-${Date.now()}.${ext}`;
      ...
      setIntroVideo(key, publicUrl);

      setMsg(
        `Intro de ${INTRO_OPTIONS.find(
          (x) => x.key === key
        )?.label ?? key} subida correctamente.`
      );
    } catch (e) {
      setErr((e as Error).message);
    } finally {
  ...
  async function save() {
    ...
    try {
      ...
      /*
       * Mantiene también el campo viejo para evitar
       * romper versiones anteriores del juego.
       */
      const legacyIntro =
        introVideos.primera_nacional ??
      ...
      await saveGameSettings({
        ...
        coimas_flags:
          settings.coimas_flags,
        anular_goles_ratio:
          settings.anular_goles_ratio,
      } as any);
  ...
  async function uploadWhistleFile(file: File) {
    ...
    try {
      if (!file.type.startsWith("audio/")) {
        throw new Error(
          "El archivo tiene que ser un audio."
        );
      }
      ...
      setMsg(
        "Pitido subido correctamente. Guardá los cambios."
      );
    } catch (e) {
      setErr((e as Error).message);
    } finally {
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <section className="rounded-2xl bg-card border border-border p-5">
        <div className="mb-5">
          ...
          <p className="text-xs text-muted-foreground">
            Cada competencia tiene su propia intro.
            No se utiliza una intro general para todas
            las divisiones.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          {INTRO_OPTIONS.map((intro) => {
            ...
            return (
              <div
                key={intro.key}
                className="rounded-xl border border-border/70 bg-background/30 p-4 space-y-3"
              >
                <div>
                ...
                {url ? (
                  <video
                    ...
                    controls
                    preload="metadata"
                    className="w-full aspect-video object-contain rounded-lg border border-border bg-black"
                  />
                ) : (
                  <div className="rounded-lg border border-dashed border-border p-5 text-center text-xs text-muted-foreground">
                    Sin intro cargada.
                    <br />
                    Se utilizará la intro animada
                    predeterminada.
                  </div>
                )}
              </div>
      ...
      <section className="rounded-2xl bg-card border border-border p-5">
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Sonido que se escucha al terminar cada
          partido.
        </p>
        ...
        <div className="space-y-2">
          ...
          {settings.whistle_audio_url && (
            <audio
              src={settings.whistle_audio_url}
              controls
      ...
      <section className="rounded-2xl bg-card border border-yellow-500/40 p-5">
        <div className="flex items-center justify-between mb-2 gap-3 flex-wrap">
          <h2 className="font-display text-xl text-yellow-500">
            💼 Coimas & Arreglos
          </h2>
          ...
          <label className="flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={settings.coimas_enabled}
              onChange={(e) =>
                setSetting(
                  "coimas_enabled",
                  e.target.checked
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Cuando está desactivado, las opciones no
          aparecen en Carrera/Torneo.
        </p>
...
function TeamEditor({
  ...
  const [form, setForm] = useState({
    ...
    division:
      (initial?.division ??
        defaultDivision ??
        "primera_nacional") as DivisionId,

    /*
     * String genérico porque:
     * - Primera Nacional -> A/B
     * - Primera C -> A/B
     * - Regional -> se usa como valor técnico compatible
     * - Federal A -> puede requerir A/B/C/D según el motor
     */
    zone: initial?.zone ?? "A",

    regional_region:
      initial?.regionalRegion ?? "",

    regional_group:
      initial?.regionalGroup ?? "",

    primary_color:
      initial?.primary ?? "#1a55a6",

    secondary_color:
      initial?.secondary ?? "#ffffff",

    stripe:
      initial?.stripe ?? "solid",
  ...
  async function uploadLogo(file: File) {
    ...
    try {
      if (!form.id) {
        throw new Error(
          "Primero asigná un ID al equipo antes de subir el escudo."
        );
      }
      ...
      if (file.size > 5 * 1024 * 1024) {
        throw new Error(
          "El escudo no puede superar 5 MB."
        );
      }
      ...
      /*
       * Un único path estable por equipo.
       * Evita que cada guardado cree un archivo diferente.
       */
      const path =
        `teams/${form.id}/logo.${ext}`;
       ...
       * también persistimos inmediatamente el logo
       * en Supabase.
       */
      if (!isNew) {
        const { error: dbError } =
  ...
  async function uploadFlags(
    ...
    if (!form.id) {
      setErr(
        "Primero asigná un ID al equipo."
      );
      return;
    }
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const path =
          `banderas/${form.id}-${Date.now()}-${Math.random()
            .toString(36)
            .slice(2, 7)}.${ext}`;
  ...
  async function uploadOne(
    ...
    const path =
      `${form.id}/${sub}/${Date.now()}-${Math.random()
        .toString(36)
        .slice(2, 8)}.${ext}`;
  ...
  async function uploadNarratorAudios(
    narratorId: string,
    files: FileList | null
  ) {
    if (!files || files.length === 0) {
  ...
  async function save() {
    ...
    try {
      if (
        ...
        throw new Error(
          "ID, nombre y abreviatura son obligatorios."
        );
      }

      /*
       * Muy importante:
       * para todas las categorías mandamos un zone
       * válido. El Regional además conserva
       * regional_region / regional_group.
       *
       * No usamos null.
       */
      const effectiveZone =
      ...
      const payload: any = {
        ...
        primera_seasons:
          form.primera_seasons === ""
            ? null
            : Number(form.primera_seasons),

        achievements:
          form.achievements || null,
      ...
      if (isNew) {
        ...
        const { data, error } =
          await supabase
            ...
            .eq("id", form.id)
            .select(
              "id,logo_url,division,zone,regional_region,regional_group"
            )
            .maybeSingle();
        ...
        if (!data) {
          throw new Error(
            "Supabase no encontró el equipo para actualizar."
          );
        }

        /*
         * Verificación real del logo.
         */
        const expectedLogo =
          form.logo_url || null;
        ...
        if (
          expectedLogo !== savedLogo
        ) {
          throw new Error(
            "El equipo se actualizó, pero el escudo no quedó guardado en Supabase."
          );
        }
      }
  ...
  return (
    <div
      className="fixed inset-0 bg-black/70 z-50 grid place-items-center p-4 overflow-auto"
      onClick={onClose}
    >
      <div
        onClick={(e) =>
        ...
        className="bg-card border border-border rounded-2xl p-6 w-full max-w-2xl"
      >
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <div className="sm:col-span-2 flex items-center gap-3">
            {form.logo_url ? (
              ...
              <div className="w-16 h-16 rounded bg-muted grid place-items-center text-xs">
                sin escudo
              </div>
            )}
          ...
          <div className="sm:col-span-2 border-t border-border pt-3">
            <label className="text-xs text-muted-foreground uppercase">
              Banderas de la hinchada
            </label>

            <div className="flex flex-wrap gap-2 mt-2">
              {form.flag_urls.length === 0 && (
                <div className="text-xs text-muted-foreground">
                  Sin banderas cargadas.
                </div>
              )}
                ...
                (url, i) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-destructive text-destructive-foreground text-xs leading-none"
                    >
                      ×
                    </button>
                  </div>
            ...
            <>
              <div>
                ...
                <Input
                  ...
                  placeholder="Norte, Cuyo, Patagonia..."
                />
              </div>
              ...
              <div className="sm:col-span-2">
                <div className="rounded-lg border border-celeste/20 bg-celeste/5 p-3 text-xs text-muted-foreground">
                  El Regional se organiza por
                  Región + Grupo. El campo técnico
                  de zona queda almacenado con un
                  valor compatible con la base para
                  evitar errores del constraint.
                </div>
              </div>
            </>
          )}
          ...
          <div className="sm:col-span-2 border-t border-border pt-3 mt-2">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={addNarrator}
                className="text-xs text-celeste underline"
              >
                + agregar relator
              </button>
            </div>
                  ...
                  >
                    ...
                    {n.urls.map(
                      (
                        ...
                        <div
                          key={i}
                          className="flex items-center gap-2 bg-muted/40 rounded p-2"
                        >
                          <audio
                            src={url}
                            controls
          ...
          <div className="sm:col-span-2 border-t border-border pt-3 mt-2">
            <label className="text-xs text-muted-foreground uppercase">
              Hinchada / música del equipo
            </label>

            <div className="space-y-2 mt-2">
              {form.hinchada_urls.length ===
                0 && (
                <div className="text-xs text-muted-foreground">
                  Sin temas.
                </div>
